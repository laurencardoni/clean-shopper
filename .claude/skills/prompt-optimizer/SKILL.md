...
description: Analyzes and optimizes prompts intended for Claude Code. Use when a user wants to improve a prompt before using it in a build session, or when evaluating whether an instruction is clear enough to produce reliable output. Evaluates clarity, context, scope, role and persona, instruction sequencing, unresolved references, and token efficiency. Trigger this skill whenever the user says things like "review my prompt", "optimize this prompt", "is this prompt good enough", "help me write a better prompt", "check this instruction", or shares a draft prompt and asks for feedback. Also trigger when the user pastes a block of text and asks if it will work well with Claude Code, or when they want to refine instructions before starting a build session.
---
